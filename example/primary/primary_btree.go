...
func (p *btEpool) get(err error, hit bool, i int, k int, q *d, t *Tree, ver uint64) *Enumerator {
	x := p.Get().(*Enumerator)
...
func (t *Tree) First() (k int, v secondary.Layer) {
	if q := t.first; q != nil {
		q := &q.d[0]
		k, v = q.k, q.v
	}
...
func (t *Tree) Get(k int) (v secondary.Layer, ok bool) {
	q := t.r
	if q == nil {
		return
	}
...
func (t *Tree) insert(q *d, i int, k int, v secondary.Layer) *d {
	atomic.AddUint64(&t.ver, 1)
	c := q.c
	if i < c {
...
// (zero-value, zero-value) if the tree is empty.
func (t *Tree) Last() (k int, v secondary.Layer) {
	if q := t.last; q != nil {
		q := &q.d[q.c-1]
		k, v = q.k, q.v
	}
...
func (t *Tree) overflow(p *x, q *d, pi, i int, k int, v secondary.Layer) {
	atomic.AddUint64(&t.ver, 1)
	l, r := p.siblings(pi)
...
func (t *Tree) Set(k int, v secondary.Layer) {
	//dbg("--- PRE Set(%v, %v)\n%s", k, v, t.dump())
	//defer func() {
	//	dbg("--- POST\n%s\n====\n", t.dump())
	//}()
...
func (t *Tree) Put(k int, upd func(oldV secondary.Layer, exists bool) (newV secondary.Layer, write bool)) (oldV secondary.Layer, written bool) {
	pi := -1
	var p *x
	q := t.r
...
func (t *Tree) split(p *x, q *d, pi, i int, k int, v secondary.Layer) {
	atomic.AddUint64(&t.ver, 1)
	r := btDPool.Get().(*d)
	if q.n != nil {
		r.n = q.n
...
// io.EOF is returned.
func (e *Enumerator) Next() (k int, v secondary.Layer, err error) {
...
// == io.EOF is returned.
func (e *Enumerator) Prev() (k int, v secondary.Layer, err error) {
	if err = e.err; err != nil {
		return
	}
	if atomic.LoadUint64(&e.ver) != atomic.LoadUint64(&e.t.ver) {
