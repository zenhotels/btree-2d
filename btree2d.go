...
type BTree2D interface {
	...
	GetLayer(key1 generic.T) (secondary.Layer, bool)
	SetLayer(key1 generic.T, layer secondary.Layer)
	ForEach(fn func(key generic.T, layer secondary.Layer) bool)
...
func (b btree2d) ForEach(fn func(key generic.T, layer secondary.Layer) bool) {
	b.primary.ForEach(fn)
}

func (b btree2d) ForEach2(key1 generic.T, fn func(key2 generic.U) bool) {
	if layer2, ok := b.primary.Get(key1); ok {
		layer2.ForEach(func(k generic.U, _ *secondary.FinalizerList) bool {
			return fn(k)
		})
	}
}

func (b btree2d) SetLayer(key1 generic.T, layer secondary.Layer) {
	b.primary.Set(key1, layer)
}

func (b btree2d) GetLayer(key1 generic.T) (secondary.Layer, bool) {
